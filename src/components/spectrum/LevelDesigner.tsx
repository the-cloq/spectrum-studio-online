      ...
      <Card className="rounded-lg border bg-card text-card-foreground shadow-sm p-4 lg:col-span-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          {levels.map((level, index) => {
            ...
            return (
              <div key={level.id} className="relative perspective">
                ...
                >
                  ...
                  <Card className="absolute inset-0 backface-hidden p-4 flex flex-col gap-2">
                    ...
                    {screensForLevel.length > 0 ? (
                      <div className="relative w-full pt-[75%] bg-muted rounded overflow-hidden">
                        <div className="absolute inset-0 flex items-center justify-center">
                          ...
                          {screensForLevel.length > 1 && (
                            <>
                              <button
                                className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 text-white px-2 py-1 text-xs rounded"
                                onClick={e => { e.stopPropagation(); prevScreen(level.id, screensForLevel); }}
                              >◀</button>

                              <button
                                className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 text-white px-2 py-1 text-xs rounded"
                      ...
                      <div className="w-full pt-[75%] bg-gray-800 rounded flex items-center justify-center text-xs text-white">
                        No screens
                      </div>
                    )}
