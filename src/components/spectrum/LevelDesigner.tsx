...
export const LevelDesigner = ({
  ...
  // --- Thumbnail renderer ---
  const drawScreenThumbnail = (canvas: HTMLCanvasElement, screen: Screen) => {
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    const thumbWidth = canvas.width;
    const thumbHeight = canvas.height;
    const tileWidth = thumbWidth / screen.width;
    const tileHeight = thumbHeight / screen.height;
    ...
    for (let y = 0; y < screen.height; y++) {
      ...
      for (let x = 0; x < screen.width; x++) {
        ...
        // Simplified: use first pixel of sprite
        const pixelColorIndex = block.sprite.pixels[0][0] ?? 0;
        ...
        ctx.fillStyle = spectrumColors[pixelColorIndex] ?? "#000";
        ctx.fillRect(
          x * tileWidth,
          y * tileHeight,
          Math.ceil(tileWidth),
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 p-4">
      ...
      <div className="lg:col-span-3">
        <Card className="rounded-lg border bg-card text-card-foreground shadow-sm p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-primary">Levels</h2>
          </div>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <canvas
                          ...
                          ref={canvas => {
                            ...
                            drawScreenThumbnail(canvas, screen);
                          }}
                        />
